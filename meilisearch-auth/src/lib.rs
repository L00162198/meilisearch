    ...
    pub fn get_key_filters(
            ...
            .map_or(false, |master_key| master_key != key.as_ref())
        {
            ...
            if !key.indexes.iter().any(|i| i.as_str() == "*") {
                filters.search_rules = match policies {
                    // Intersect policies with parent key authorized indexes.
                    Some(policies) => SearchRules::Map(
                        key.indexes
                            .into_iter()
                            .filter_map(|index| {
                                policies
                                    .get_index_policy(&index)
                                    .map(|policy| (index, Some(policy)))
                            })
                            .collect(),
                    ),
